  ...
  const renderCalendar = () => {
    ...
    for (let week = 0; week < 6; week++) {
      ...
      for (let i = 0; i < 7; i++) {
        const cellIndex = week * 7 + i;
        if (cellIndex < firstDay || day > daysInMonth) {
          days.push(<td key={`${week}-${i}-empty`} className="p-0.5 sm:p-2"></td>);
        } else {
          const dateKey = formatDate(year, month, day);
          const category = getBookingCategory(dateKey);
          const isToday =
          ...
          const todayHighlight = isToday
            ...
            : "border-gray-300";
          const selectedHighlight = isSelected
            ? "bg-blue-200 border-blue-800 shadow-xl scale-[1.03]"
            : "bg-white";
          days.push(
            <td key={`${week}-${i}-${day}`} className="p-0.5 sm:p-2">
              <div className={`w-9 h-9 xs:w-10 xs:h-10 sm:w-12 sm:h-12 md:w-14 md:h-14 lg:w-16 lg:h-16 rounded-lg flex flex-col items-center justify-center cursor-pointer transition-all duration-300 ease-in-out ${selectedHighlight} ${todayHighlight} border-2 hover:bg-amber-50 hover:border-amber-300 hover:shadow-md`} onClick={() => { setSelectedDate(dateKey); }}>
                <span className="text-xs xs:text-sm sm:text-base md:text-lg font-semibold text-gray-800">{day}</span>{category && (<span className={`mt-0.5 w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full ${dotColor}`}></span>)}
              </div>
            </td>
          );
          day++;
